...
public class MethodLocalVariable {
      ...
      *
      *매개변수
      *-메서드 호출시 외부에서 전달받는 값을 저장하는 지역 변수
      *-메서드 선언부에 정의됨
...
System.out.println("say실행 후 : "+name);
		if(name.equals("철수")) {
		 // 	String name ="지원"; 포함되는 관계는 같은 지역이므로
			//동일한 이름을 사용할 수 없다
		}
	...
	//기본형
		int num = 10;
		changeValue(num);
		System.out.println("main에서 changValue 호출후: " +num );
	...
	public static void changeReference(int[] array) {
		...
		for(int n :array) {
		System.err.print(n+ " ");
	} //참조형을 전달하면 실제 값자체를 전달하지않고
      //참조형에 담긴 주소값을 전달하기 때문에
	  // 해당주소로 접근해서 원본데이터를 변경할 수 있다.
		System.out.println();
		array[0] = 100;
