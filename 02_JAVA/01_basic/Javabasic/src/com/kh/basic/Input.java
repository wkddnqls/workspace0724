...
//-----------------------------------------------------------------------------		
		...
		System.out.print("주소: ");
		String address = sc.nextLine();  // 문자열\n
		
		System.out.print("나이: ");
		int age = sc.nextInt(); // 정수일때   정수\n 정수값만 가져가다보니 \n를 못가져감
		sc.nextLine(); // 버퍼에남아있는 nextInt()를 입력할때 발생한 \n(엔터값)을 비워주는코드
		
		System.out.printf("제 이름은 %s이며 %s에 살고 %d살 입니다 " , name , address,age );
        // 문자 이외에 원시타입을 입력받을 때
		//.next타입()
		// nextInt() , nextDouble() , nextBoolean() ....
		
		sc.close(); //Scanner 자원을 다쓰고 반납 *한번 반납하면 다시 사용x 
