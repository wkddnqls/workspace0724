...
public class ServeStream {
   /*
    * 보조스트림
    * 외부매체와 직접연결되지않고, 기반스트림을 감싸서 기능/성능을 높여줌
...
/*
 * 직렬화 
 * 객체는 메모리안에 존재하는 인스턴스 개념이기 떄문에 그대로 전송할 수 없음
 * 직렬화 후 이진데이터로 외부에 전송 가능
 * - 객체(오브젝트)를 바이트 형대로 변환하는 과정
 * - 파일에 저장하거나 네트워크로 전송할 수 있도록함.
 ...
 */
 // 프로그램 -> 파일(출력) : 객체(직렬화)
		...
		public void ObjectSave() {
			Product p1 = new Product("아이폰 16 ", 100000);
			Product p2 = new Product("아이폰 16 Pro ", 200000);
			Product p3 = new Product("아이폰 16mini ", 300000);
		...
		public void ObjectRead() {
			...
			try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream("product1.dat"))){
				...
				System.out.println("객체읽기중 I O 에러발생 ");
				e.printStackTrace();
			}catch (ClassNotFoundException e) {
				System.out.println("직렬화 대상 클래스 찾을 수 없습니다 . ");
				e.printStackTrace();
			}
		}
}