...
public class StringAPI {
  public void method01() {
	  ...
	  // String 클래스에 toString 메서드는 이미 오버라이딩 되어있어
	  // 그래서 참조변수 출력시 Object의 toString (타입@주소값16진수)이 아니라 
	   //   String 의 저장된 문자열이 반환됨
	  
	  System.out.println(str1==str2);
	  //String객체의 주소값을 비교하기 때문에 false가 나옴 
	  System.out.println(str1.equals(str2));
	  //String 클래스에서 equals메서드도 이미 오버라이딩 되어있음 (주소값 비교 x 문자열값 비교 o)
	  
	  System.out.println(str1.hashCode());
	  System.out.println(str2.hashCode());
	//String클래스에서 hashCode메서드도 이미 오버라딩 되어있음(주소값이 아닌 문자열을 가지고 해시값을
      // 만들도록 ) 
	  
	  //System.identityHashCode(참조변수)
	  //해당참조변수의 메모리값을 기반으로한 해시값을 반환
	  ...
	  //문자열.substring(시작위치,[끝위치})
	    ...
	    System.out.println("모든 준자열 대문자로 변환 : " + str1.toUpperCase());
	    
	    System.out.println("모든 준자열 소문자로 변환 : " + str1.toLowerCase());
	    ...
	    public void method03() {
	    	...
	    	//방법1. 분리된 문자열들을 String[]에 차곡차곡 담아야할때
	    ...
	    System.out.println(stn.nextToken());
	    System.out.println(stn.nextToken());
	    System.out.println(stn.countTokens()); // 토큰의 남은개수
	    System.out.println(stn.nextToken());
	    System.out.println(stn.nextToken());
	    System.out.println(stn.nextToken());
	    //System.out.println(stn.nextToken()); 더이상 토큰이 남아있지않을떄 다음 토큰 호출시 에러발생
	    ...
	    stn = new StringTokenizer(str,",");
	    while( stn.hasMoreElements()) { //토큰남은거잇냐?
	    	System.out.println(stn.nextToken());
	    }
	  }
