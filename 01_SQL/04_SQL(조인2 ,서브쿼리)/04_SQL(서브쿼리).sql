 ...
 *서브쿼리 구문
  서브쿼리를 수행한 결과값이 몇행, 몇열로 나오느냐에 따라서 분류
    -- 단일행 서브쿼리 : 서브쿼리의 조회 결과값이 오로지 한개일때
...
FROM EMPLOYEE
WHERE SALARY = (SELECT MIN(SALARY)Y FROM EMPLOYEE);
...
--2) 대리집급임에도 과장직급의 최소급여보다 더 많이 받는사원들의 사번,이름,직급,급여 조회
...
--다중열 서브쿼리
--튜플비교 : 2개 이상의 열(컬럼)을 조합해서 비교하는 방식
SELECT EMP_NAME, DEPT_CODE , JOB_CODE, HIRE_DATE
FROM EMPLOYEE
...
ORDER BY 연봉 DESC ; 
--LIMIT 5; 오라클에서 지원 X
-- ROWNUM : 오라클에서 기본적으로 제공해주는 컬럼, 조회된 순서대로 1부터 순번 부여
...
--> ORDER BY 절이 수행된 결과를 가지고 ROUNUM 부여 : 상위 5명
SELECT ROWNUM , EMP_NAME , SALARY
...
SELECT DEPT_CODE , "돈"
FROM
(SELECT DEPT_CODE , FLOOR (AVG(SALARY)) AS "돈"
FROM EMPLOYEE
GROUP BY DEPT_CODE
ORDER BY "돈" DESC)
WHERE ROWNUM <=3;
