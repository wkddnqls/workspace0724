...
/*
 2. 외부 조인 (OUTER JOIN) /포괄조인
 두 테이블간의 JOIN시 일치하지 않는 행도 포함시켜 조회가능 
 단 , 반드시 LEEFT / RIGHT 를 지정해야한다 ( 어떤 테이블을 기준으로 할것인가)
...
-- 1) LEFR OUTER JOIN : 두 테이블 중 왼편에 기술된 테이블을 기준으로 JOIN
SELECT EMP_NAME , DEPT_TITLE , SALARY , SALARY *12
FROM EMPLOYEE
...
-- FULL [OUTER] JOIN : 두테이블이 가진 모등행을 조회할 수 있다.
...
/*
내추럴 조인(NATURAL JOIN)
두 테이블간에 공통된 컬럼이름을 기준 정해서 자동으로 조인하는 방식
