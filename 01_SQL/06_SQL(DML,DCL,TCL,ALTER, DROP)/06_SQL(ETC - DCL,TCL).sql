...
/*
   <TCL : 트렌잭션 제어문>
   트랜잭션은 하나이상의 DML(INSERT, UPDATE , DELETE)문 묶어서 하나의 논리적 작업
   단위로 처리하는 개념
   
   DML문 한개를 수행할 떄 트랜잭션이 존재하지않으면 트랜잭션을 만들어서 묵음,
   ...
   대부분 요즘은 AUTO COMMIT이 설정되어 있어서 DML을 전달할 때마다 자동으로 커밋이 됨. 
...
-- AUTOCOMMIT 수동설정 /ON ->자동으로 DML실행시마다 COMMTI이 진행됨.
SET AUTOCOMMIT OFF;
...
/*
   ...
   I(Isolatoion) 독립성 : 여러 트랜잭션이 동시에 실행되도 서로 간섭없이 독립적으로 실행되어야한다.
