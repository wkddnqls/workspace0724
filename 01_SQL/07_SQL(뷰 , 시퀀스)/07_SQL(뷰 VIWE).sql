/*
   ...
   복잡한 JOIN쿼리를 매번 작성하지않고 간단하게 재사용할 수 있음.
...
-- 뷰 호출
-- 실제 실행되는 것은 서브쿼리 (인라인뷰 형태)
SELECT *
FROM VW_EMPLOYEE;

--한국에서 근무하는 사원들의 사번, 이름, 부서명, 급여, 근무국가조회

SELECT * 
FROM VW_EMPLOYEE
WHERE NATIONAL_NAME = '한국' ;

--러시아에서 근무하는 사원들의 사번, 이름, 부서명, 급여, 근무국가조회

SELECT * 
FROM VW_EMPLOYEE
WHERE NATIONAL_NAME = '러시아' ;

--일본에서 근무하는 사원들의 사번, 이름, 부서명, 급여, 근무국가조회
...
--VIEW가 없을 때는 생성, 이미 존재한다면 수정할 수 있음. (덮어씌우기, OR 수정해서 덮기)
...
-----------------------------------------------------------------------------
/*
    뷰 컬럼에 연산식이 있는 경우 무조건 별칭을 지정해야 함.
*/
...
AS(SELECT JOB_CODE , JOB_NAME
    ...
    SELECT *FROM VW_JOB;  --가상 논리 테이블 (실제데이터가 저장되어있지않음)
    SELECT * FROM JOB ; 

INSERT INTO VW_JOB VALUES('J9' , '여행');
...
/*
     DML 명령어 사용이 불가한 경우
     1)뷰에 없는 컬럼을 조작
     2)뷰에 정의되어있지 않은 컬럼중에 NOT NULL 제약조건이 있는 컬럼이 잇는 경우
     3)산술/함수식을 사용한 경우
...
*/
 ...
 /*
      ...
      > WITH CHECK OPTION : 뷰에 WHERE 조건을 강제하여 조건에 맞는 데이터만
      삽입/수정 가능하게함
      
      >WITH READ ONIY : 뷰에대해서 조회만 가능하도록.
 
 
 */
 
 CREATE OR REPLACE NOFORCE VIW VI_EMP
 AS ( SELECT TOCODE,TENEM, TOCONTETN)
 FORM TT);
 SELECT * FROM VI_EMP;
  
  
  CREATE OR REPLACE FORCE VIEW VI_EMP
 AS ( SELECT TOCODE,TNAEM, TOCONTETN
 FROM TT);
