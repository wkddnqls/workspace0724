...
-- 3 국어국문학과에 다니는 여학생 중 현재 휴학중인 여학생을 찾아달라는 요청이왓다
-- 누구인가 (국문학과의 학과코드는 학과 테이블을 조회해서 찾아내자)
SELECT STUDENT_NAME 
FROM TB_STUDENT
...
FROM TB_PROFESSOR
WHERE NOT(DEPARTMENT_NO IS NOT NULL);

--7 혹시 전산상의 착오로 학과가 지정되어 있지않은 학생이 있는지 확인하고자 한다.
